...
def extract_resnet50_features(images):
    ...
    feature_vectors = [model.predict(
        tf.keras.applications.resnet50.preprocess_input(cv2.resize(image, (224, 224))).reshape(1, 224, 224,
                                                                                               3)).flatten()
        ...
        if image_files[i] not in processed_images:  # Check if the image is already processed
            ...
            for j in range(num_images):
                if i != j:
                    ...
                    if similarity_score > 0.80:  # Adjust the threshold as needed
                        group.append(image_files[j])
                        processed_images.add(image_files[j])  # Add processed images to the set
            if group:
                group.append(image_files[i])  # Add the current image to the group
                similar_images.append(group)
                processed_images.add(image_files[i])  # Add processed images to the set
